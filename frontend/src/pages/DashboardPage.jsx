...
import { TrendingUp, Users, Eye, MousePointerClick, FileText, Clock, CheckCircle } from 'lucide-react';
...
const STATUS_COLORS = { published: 'badge-green', scheduled: 'badge-yellow', draft: 'badge-gray', failed: 'badge-red', publishing: 'badge-blue' };
...
export default function DashboardPage() {
  const { data: overview } = useQuery({ queryKey: ['analytics-overview'], queryFn: () => analyticsAPI.overview({ days: 30 }) });
  const { data: content } = useQuery({ queryKey: ['content-recent'], queryFn: () => contentAPI.list({ limit: 6 }) });
  const { data: topContent } = useQuery({ queryKey: ['top-content'], queryFn: () => analyticsAPI.topContent({ limit: 5 }) });

  const platformData = overview?.byPlatform?.map(p => ({
    name: p._id, reach: Math.round(p.totalReach / 1000),
  })) || [];
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard icon={Eye} label="Total reach" value={overview?.summary?.totalReach ? `${(overview.summary.totalReach / 1000).toFixed(0)}K` : '—'} change="↑ 18% vs last month" color="indigo" />
        <StatCard icon={TrendingUp} label="Impressions" value={overview?.summary?.totalImpressions ? `${(overview.summary.totalImpressions / 1000000).toFixed(1)}M` : '—'} change="↑ 22%" color="blue" />
        <StatCard icon={MousePointerClick} label="Engagement rate" value={overview?.summary?.avgEngagementRate ? `${overview.summary.avgEngagementRate}%` : '—'} change="↑ 0.8%" color="green" />
        <StatCard icon={FileText} label="Posts published" value={content?.total || '—'} change="This month" color="purple" />
      </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {item.scheduledAt && (
                  <span className="flex items-center gap-1 text-xs text-gray-400">
                    <Clock size={11} /> {format(new Date(item.scheduledAt), 'MMM d, h:mm a')}
                  </span>
                )}
                <span className={STATUS_COLORS[item.status] || 'badge-gray'}>{item.status}</span>
              </div>
            </div>
          ))}
          {!content?.items?.length && (
            <div className="text-center py-8 text-sm text-gray-400">No content yet. <Link to="/content/new" className="text-indigo-600">Create your first post →</Link></div>
          )}
        </div>
      </div>
    </div>
